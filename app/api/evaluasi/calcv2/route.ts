...
const TIMEFRAME = "5m";
const EVALUATOR_VERSION = "v2-5m-78bars-yahoo-validation-WIB";
...
async function fetchOHLC5m(kode: string, fromDateTime: Date) {
  ...
  try {
    ...
    const candles = timestamps.map((t: number, i: number) => ({
      ...
      dateWIB: new Date(t * 1000 + 7*3600*1000).toISOString().slice(0,10),
      timeWIB: new Date(t * 1000 + 7*3600*1000).toISOString().slice(11,16),
...
function evaluateSignal5m(sig: Signal, ohlcData: any) {
  ...
  // LOCK: SIGNAL TIMESTAMP = tanggal + jam + WIB
  const signalTimeStr = `${sig.tanggal}T${sig.jam || "00:00"}:00+07:00`;
  ...
  const provenance = {
    ...
    signal_timestamp_wib: `${sig.tanggal} ${sig.jam || "00:00"} WIB`,
    signal_timestamp_raw: `${sig.tanggal} ${sig.jam || "00:00"}`,
    first_eval_timestamp: slice[0]?.dateTime || null,
    first_eval_timestamp_wib: slice[0]? `${slice[0].dateWIB} ${slice[0].timeWIB} WIB` : null,
    last_eval_timestamp: slice[slice.length - 1]?.dateTime || null,
    last_eval_timestamp_wib: slice[slice.length - 1]? `${slice[slice.length-1].dateWIB} ${slice[slice.length-1].timeWIB} WIB` : null,
  ...
  if (slice.length === 0) {
    return {
      ...
      hitDate: null, hitTime: null, days: 0, bars: 0,
      exitPrice: null, returnPct: null,
      note: `Belum ada candle 5m setelah signal ${sig.jam || "00:00"} WIB - Yahoo 5m hanya 7 hari`,
      provenance,
    };
  }
  ...
  for (let i = 0; i < slice.length; i++) {
    ...
    if (hitTP && hitSL) {
      return {
        ...
        hitDate: d.dateWIB, hitTime: d.timeWIB, days: i + 1, bars: i + 1,
        exitPrice: null, returnPct: null,
        note: `AMBIGUOUS 5m ${d.dateWIB} ${d.timeWIB} WIB: H=${d.high} L=${d.low} vs TP=${sig.tp} SL=${sig.sl}`,
    ...
    if (hitTP) {
      ...
      return {
        ...
        hitDate: d.dateWIB, hitTime: d.timeWIB, days: i + 1, bars: i + 1,
        exitPrice, returnPct,
        note: `${sig.action} TP_HIT 5m ${d.dateWIB} ${d.timeWIB} WIB ${sig.action === "BUY"? `H=${d.high}` : `L=${d.low}`} >= TP ${sig.tp}`,
        provenance: {...provenance, hit_bar: d, hit_bar_index: i },
      };
    }
    if (hitSL) {
      ...
      return {
        ...
        hitDate: d.dateWIB, hitTime: d.timeWIB, days: i + 1, bars: i + 1,
        exitPrice, returnPct,
        note: `${sig.action} SL_HIT 5m ${d.dateWIB} ${d.timeWIB} WIB ${sig.action === "BUY"? `L=${d.low}` : `H=${d.high}`} vs SL ${sig.sl}`,
        provenance: {...provenance, hit_bar: d, hit_bar_index: i },
      };
    }
  }
  ...
  if (slice.length < TIMEOUT_BARS) {
    return {
      outcome: "OPEN" as const,
      hitDate: null, hitTime: null, days: slice.length, bars: slice.length,
      exitPrice: null, returnPct: null,
  ...
  return {
    ...
    hitDate: last.dateWIB, hitTime: last.timeWIB, days: TIMEOUT_BARS, bars: TIMEOUT_BARS,
    exitPrice, returnPct,
...
export async function GET() {
  ...
  for (const sig of signals) {
    const signalTime = new Date(`${sig.tanggal}T${sig.jam || "00:00"}:00+07:00`);
  ...
  const closedArr = results.filter(r => ["TP_HIT", "SL_HIT", "TIMEOUT"].includes(r.outcome));
  ...
  const winRate = closedArr.length? (tp / closedArr.length * 100) : 0;
  const avgReturn = closedArr.length? closedArr.reduce((a, b) => a + (b.returnPct || 0), 0) / closedArr.length : 0;
  ...
  return NextResponse.json({
    summary: {
      ...
      timeout_bars: TIMEOUT_BARS,
      evaluator_version: EVALUATOR_VERSION,
      engine_type: "V2_ENGINE_VALIDATION - Yahoo 5m WIB - bukan historical track record",
      formula: "BUY: TP=High>=TP SL=Low<=SL | SELL: TP=Low<=TP SL=High>=SL | FIRST HIT 5m WIB | TIMEOUT=Close bar 78"
    },
